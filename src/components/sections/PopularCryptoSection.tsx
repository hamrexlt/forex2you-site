import React from "react";

const PopularCryptoSection = () => {
  return (
    <div className="css-m0ecq1">
      <div
        className="css-6sm2ml"
        data-aos="flip-left"
        data-aos-delay="500"
        data-aos-easing="linear"
      >
        <div className="css-1leavdy">
          <div className="css-lzsise">
            <div data-bn-type="text" className="css-rlhgi">
              Popular cryptocurrencies
            </div>
          </div>
          <a className="css-buqk49" href="https://app.forex2you.org">
            <div data-bn-type="text" className="css-1c82c04">
              View more markets
            </div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              fill="none"
              className="css-1cizdsm"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M12.288 12l-3.89 3.89 1.768 1.767L15.823 12l-1.768-1.768-3.889-3.889-1.768 1.768 3.89 3.89z"
                fill="currentColor"
              />
            </svg>
          </a>
        </div>
        <div className="css-1jsfmns">
          <div className="css-1mi1v6l">
            <div
              data-bn-type="text"
              title="Name"
              style={{ overflow: "hidden", WebkitLineClamp: 1 }}
              className="css-1i04fkn"
            >
              Name
            </div>
          </div>
          <div className="css-1gf2u02">
            <div
              data-bn-type="text"
              title="Last Price"
              style={{ overflow: "hidden", WebkitLineClamp: 1 }}
              className="css-1i04fkn"
            >
              Last Price
            </div>
          </div>
          <div className="css-1mnwmjc">
            <div
              data-bn-type="text"
              title="24h Change"
              style={{ overflow: "hidden", WebkitLineClamp: 1 }}
              className="css-1i04fkn"
            >
              24h Change
            </div>
          </div>
          <div className="css-m65ilq">
            <div
              data-bn-type="text"
              title="24h Volume"
              style={{ overflow: "hidden", WebkitLineClamp: 1 }}
              className="css-1i04fkn"
            >
              24h Volume
            </div>
          </div>
          <div className="css-bxbx2u">
            <div
              data-bn-type="text"
              title="Market Cap"
              style={{ overflow: "hidden", WebkitLineClamp: 1 }}
              className="css-1i04fkn"
            >
              Market Cap
            </div>
          </div>
        </div>
        <a
          id="top_crypto_table-1-BNB_USDT"
          className="css-1k26pp0"
          href="https://app.forex2you.org"
        >
          <div className="css-1mi1v6l">
            <div
              data-round="true"
              className="mica-lazy-img css-zk8p0o"
              data-src="/images/94863af2-c980-42cf-a139-7b9f462a36c2.png"
              style={{
                backgroundColor: "transparent",
                backgroundImage:
                  'url("/images/94863af2-c980-42cf-a139-7b9f462a36c2.png")',
                backgroundRepeat: "no-repeat",
                backgroundSize: "100%",
                backgroundPosition: "center center",
              }}
            />
            <div className="css-1avbqo0">
              <div data-bn-type="text" className="css-bnaz7g">
                BNB
              </div>
              <div
                data-bn-type="text"
                title="BNB"
                style={{ overflow: "hidden", WebkitLineClamp: 1 }}
                className="css-101w6jk"
              >
                BNB
              </div>
            </div>
          </div>
          <div className="css-1gf2u02">
            <div className="css-10nf7hq">
              <div data-bn-type="text" className="css-1dqq3gy">
                $311.6
              </div>
            </div>
          </div>
          <div className="css-xtcmd">
            <div
              data-bn-type="text"
              style={{ direction: "ltr" }}
              className="css-139a76i"
            >
              +0.81%
            </div>
          </div>
          <div className="css-m65ilq">
            <div data-bn-type="text" className="css-jgts3k">
              491,858.36
            </div>
          </div>
          <div className="css-bxbx2u">
            <div data-bn-type="text" className="css-jgts3k">
              $48,565M
            </div>
          </div>
        </a>
        <a
          id="top_crypto_table-2-BTC_USDT"
          className="css-1k26pp0"
          href="https://app.forex2you.org"
        >
          <div className="css-1mi1v6l">
            <div
              data-round="true"
              className="mica-lazy-img css-zk8p0o"
              data-src="/images/87496d50-2408-43e1-ad4c-78b47b448a6a.png"
              style={{
                backgroundColor: "transparent",
                backgroundImage:
                  'url("/images/87496d50-2408-43e1-ad4c-78b47b448a6a.png")',
                backgroundRepeat: "no-repeat",
                backgroundSize: "100%",
                backgroundPosition: "center center",
              }}
            />
            <div className="css-1avbqo0">
              <div data-bn-type="text" className="css-bnaz7g">
                Bitcoin
              </div>
              <div
                data-bn-type="text"
                title="BTC"
                style={{ overflow: "hidden", WebkitLineClamp: 1 }}
                className="css-101w6jk"
              >
                BTC
              </div>
            </div>
          </div>
          <div className="css-1gf2u02">
            <div className="css-10nf7hq">
              <div data-bn-type="text" className="css-1dqq3gy">
                $27,653
              </div>
            </div>
          </div>
          <div className="css-xtcmd">
            <div
              data-bn-type="text"
              style={{ direction: "ltr" }}
              className="css-139a76i"
            >
              +0.34%
            </div>
          </div>
          <div className="css-m65ilq">
            <div data-bn-type="text" className="css-jgts3k">
              50,887.55
            </div>
          </div>
          <div className="css-bxbx2u">
            <div data-bn-type="text" className="css-jgts3k">
              $536,101M
            </div>
          </div>
        </a>
        <a
          id="top_crypto_table-3-ETH_USDT"
          className="css-1k26pp0"
          href="https://app.forex2you.org"
        >
          <div className="css-1mi1v6l">
            <div
              data-round="true"
              className="mica-lazy-img css-zk8p0o"
              data-src="/images/3a8c9fe6-2a76-4ace-aa07-415d994de6f0.png"
              style={{
                backgroundColor: "transparent",
                backgroundImage:
                  'url("/images/3a8c9fe6-2a76-4ace-aa07-415d994de6f0.png")',
                backgroundRepeat: "no-repeat",
                backgroundSize: "100%",
                backgroundPosition: "center center",
              }}
            />
            <div className="css-1avbqo0">
              <div data-bn-type="text" className="css-bnaz7g">
                Ethereum
              </div>
              <div
                data-bn-type="text"
                title="ETH"
                style={{ overflow: "hidden", WebkitLineClamp: 1 }}
                className="css-101w6jk"
              >
                ETH
              </div>
            </div>
          </div>
          <div className="css-1gf2u02">
            <div className="css-10nf7hq">
              <div data-bn-type="text" className="css-1dqq3gy">
                $1,892
              </div>
            </div>
          </div>
          <div className="css-xtcmd">
            <div
              data-bn-type="text"
              style={{ direction: "ltr" }}
              className="css-139a76i"
            >
              +2.19%
            </div>
          </div>
          <div className="css-m65ilq">
            <div data-bn-type="text" className="css-jgts3k">
              421,876.38
            </div>
          </div>
          <div className="css-bxbx2u">
            <div data-bn-type="text" className="css-jgts3k">
              $227,521M
            </div>
          </div>
        </a>
        <a
          id="top_crypto_table-4-GAL_USDT"
          className="css-1k26pp0"
          href="https://app.forex2you.org"
        >
          <div className="css-1mi1v6l">
            <div
              data-round="true"
              className="mica-lazy-img css-zk8p0o"
              data-src="/images/4e3c263c-0899-4640-9f50-ad4fcd1f4bd1.png"
              style={{
                backgroundColor: "transparent",
                backgroundImage:
                  'url("/images/4e3c263c-0899-4640-9f50-ad4fcd1f4bd1.png")',
                backgroundRepeat: "no-repeat",
                backgroundSize: "100%",
                backgroundPosition: "center center",
              }}
            />
            <div className="css-1avbqo0">
              <div data-bn-type="text" className="css-bnaz7g">
                Galxe
              </div>
              <div
                data-bn-type="text"
                title="GAL"
                style={{ overflow: "hidden", WebkitLineClamp: 1 }}
                className="css-101w6jk"
              >
                GAL
              </div>
            </div>
          </div>
          <div className="css-1gf2u02">
            <div className="css-10nf7hq">
              <div data-bn-type="text" className="css-1dqq3gy">
                $1.39
              </div>
            </div>
          </div>
          <div className="css-6ug0i1">
            <div
              data-bn-type="text"
              style={{ direction: "ltr" }}
              className="css-139a76i"
            >
              -1.14%
            </div>
          </div>
          <div className="css-m65ilq">
            <div data-bn-type="text" className="css-jgts3k">
              2.00M
            </div>
          </div>
          <div className="css-bxbx2u">
            <div data-bn-type="text" className="css-jgts3k">
              $104M
            </div>
          </div>
        </a>
        <a
          id="top_crypto_table-5-GMT_USDT"
          className="css-1k26pp0"
          href="https://app.forex2you.org"
        >
          <div className="css-1mi1v6l">
            <div
              data-round="true"
              className="mica-lazy-img css-zk8p0o"
              data-src="/images/38ece034-cbaa-44eb-89ce-42013145f9f7.png"
              style={{
                backgroundColor: "transparent",
                backgroundImage:
                  'url("/images/38ece034-cbaa-44eb-89ce-42013145f9f7.png")',
                backgroundRepeat: "no-repeat",
                backgroundSize: "100%",
                backgroundPosition: "center center",
              }}
            />
            <div className="css-1avbqo0">
              <div data-bn-type="text" className="css-bnaz7g">
                Green Metaverse Token
              </div>
              <div
                data-bn-type="text"
                title="GMT"
                style={{ overflow: "hidden", WebkitLineClamp: 1 }}
                className="css-101w6jk"
              >
                GMT
              </div>
            </div>
          </div>
          <div className="css-1gf2u02">
            <div className="css-10nf7hq">
              <div data-bn-type="text" className="css-1dqq3gy">
                $0.2671
              </div>
            </div>
          </div>
          <div className="css-6ug0i1">
            <div
              data-bn-type="text"
              style={{ direction: "ltr" }}
              className="css-139a76i"
            >
              -0.78%
            </div>
          </div>
          <div className="css-m65ilq">
            <div data-bn-type="text" className="css-jgts3k">
              22.30M
            </div>
          </div>
          <div className="css-bxbx2u">
            <div data-bn-type="text" className="css-jgts3k">
              $231M
            </div>
          </div>
        </a>
        <div className="css-vurnku">
          <div data-bn-type="text" className="css-xhdd21">
            Sign up now to build your own portfolio for free!
          </div>
          <a
            data-bn-type="button"
            className=" css-y69muh"
            href="https://app.forex2you.org"
          >
            Get Started
          </a>
        </div>
      </div>
    </div>
  );
};

export default PopularCryptoSection;
